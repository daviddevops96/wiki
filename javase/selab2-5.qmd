...
This line of code is creating an ``InputStream`` that reads from a ``byte array`` **containing the bytes of the input string** using the platform's default charset, which allow to use this ``InputStream`` **as regular input stream.**
