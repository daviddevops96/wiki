...
This line of code is the **key-solution to use InputStream to simulate a input console**.

It creates an ``InputStream`` ``in`` that reads from a ``byte array`` **containing the bytes of the input string** ``input.getBytes()`` (using the platform's default charset), which allow us to use this ``InputStream`` **as regular input stream.**
