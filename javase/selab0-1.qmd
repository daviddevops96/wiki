...
categories: [javase, lab , class, object, composition]
toc: true
...
4. Create ``Junit`` tests to test objects. Use ``Faker`` to create objects:
    ...
    2. Test ``createCar``: check weather the object ``Car`` creation works properly.
    3. Test ``createBook``: check weather the object ``Book`` creation works properly.
    4. Test ``createStudent``: check weather the object ``Student`` creation works properly
...
5. Create ``Junit`` tests to test operations:
    ...
    1. Test ``methodsPerson``: check weathe methods ``Person`` works properly: ``becomeOlder()``, ``killVampire()``, ``withFirstName()``
...
Here, the `Car` class represents a **car** with a doors, places and others. 
...
<br>
[{{< fa solid arrow-left >}} Java Patterns: Builder](/javase/se-pattern-builder.qmd)
<br>
</div>
<br>
...
<br>
[Lab#SE00-2: Maven Clinic {{< fa solid arrow-right >}}](/javase/selab0-2.qmd)
<br>
</div>
<br>
</div>
